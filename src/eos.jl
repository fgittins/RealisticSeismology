"""
Equation-of-state models in β equilibrium at constant temperature.
...
tabulated form. Tables are initially three-dimensional in temperature `T`
[MeV], baryon-number density `nb` [fm^-3] and electron fraction `Yₑ`
[dimensionless].

Tables are put in two-dimensional form (`data/eos/apr.table`,
`data/eos/dd2.table`) using `scripts/generate_table.jl` by enforcing β
equilibrium, solving for `Yₑ` at each `(T, nb)`. Then, models are interpolated
in these two dimensions.

This implementation takes `T` to be constant. Baryon-number density is
determined by inverting `p = p(nb)`.
...
function EOS(T, model, n, m)
    ...
    EOS(itp₁, itp₂, itp₃, itp₄, itp₅, itp₆, itp₇, itp₈, itp₉, itp₁₀,
        ymin, ymax, T, p, nb)
end

"APR equation of state at constant temperature."
function APR(T)
    n, m = 132, 220
...
"DD2 equation of state at constant temperature."
function DD2(T)
    n, m = 80, 323
...
function nb(eos, p)
    prob = IntervalNonlinearProblem{false}(
            (lognb, (eos, logT, logp)) -> eos.itp₁(logT, lognb) - logp,
